...
class SurveyService(SalaryServiceMixin):
    ...
    async def get_all(self, filters: dict[str, str], sorts=list[str]) -> list[SurveyResultSchema]:
        ...
        return [SurveyResultSchema.from_orm(result) for result in
            await self.repository.get_all(filters=filter_clauses, sorts=sort_clauses)]
